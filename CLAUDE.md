...
- **Errors:** one `snafu` enum, `HeuremaError` (`#[non_exhaustive]`), with `#[snafu(implicit)] location` on every variant. Inside `heurema`, errors are built through context selectors (`SnapshotFormatSnafu { .. }.build()`, `ensure!`); the selectors are crate-private, so the adapter crates construct variants directly with `location: std::panic::Location::caller()`. `PersistenceSource` type-erases only at the backend boundary.
...
Phases are numbered 00–04. Earlier docs numbered them 1–3: old Phase 1 (API, RRF, typed placeholder engines) is the pre-00 scaffold, old Phase 2 (fresh engines) is Phase 01, and old Phase 3 (the `atmis` / `thesauros` adapters) shipped with the scaffold. Those labels are retired.
...
- Always: keep the index API engine-agnostic — heurēma must not learn about SQL or consumer-specific tuple shapes. Datalog lives here only as `akolouthia`, heurēma's own engine, never as a consumer's query language.
