...
incremented — there is no index behind either type yet. `PersistenceBackend` (`src/persistence.rs`) is
...
- `PersistenceBackend` - save / load named vector and FTS indexes; backend-agnostic. Trait only, no implementation.
